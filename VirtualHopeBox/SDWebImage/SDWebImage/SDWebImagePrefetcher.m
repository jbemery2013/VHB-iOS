...
- (void)reportStatus
{
    ...
    NSLog(@"Finished prefetching (%li successful, %li skipped, timeElasped %.2f)", (long)total - _skippedCount, (long)_skippedCount, CFAbsoluteTimeGetCurrent() - _startedTime);
}
...
{
    _finishedCount++;
    NSLog(@"Prefetched %li out of %li", (long)_finishedCount, (long)[self.prefetchURLs count]);
...
{
    _finishedCount++;
    NSLog(@"Prefetched %li out of %li (Failed)", (long)_finishedCount, (long)[self.prefetchURLs count]);
