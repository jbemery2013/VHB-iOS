...
//                                           !   !   !   !   !   !   !   !   !   !   !   !   !   !   !   !
NSString * const T2Bits = @"x'd6cacacf9c1078dcc0717e1221274fefdc8743ed60097e79909cc8086e4f0733'";

// Encryption PIN
NSString * const T2P = @"aToZ";
...
void FIPS_flip()
{
    ...
    if(1 == ret && 0 != mode)
    {
        ...
        memmove(dataBytes, testBlock, sizeof(testBlock));
        ...
        do {
            ...
            // Seed the key
            memmove(keyBytes, FIPS_keyClue, sizeof(FIPS_keyClue));
...
NSString * secureGetStringForKeyPlist(NSString *p, NSString *k, NSDictionary *d) {
    ...
    NSString *encryptedValue = [d objectForKey:encryptedKey];

    // Return the decrypted value
    return dRaw(p, encryptedValue);
}

// Encrypt the specified String value and write it to the plist (i.e.; NSDictionary)
// encryptedSaveStringForKeyPlist
void secureSaveStringForKeyPlist(NSString *p, NSString *v, NSString *k, NSMutableDictionary *d) {
    // Generate the encrypted key (using the pin and the unencrypted value of the key)
    NSString *encryptedKey = eRaw(p, k);
...
NSNumber * secureGetNumberForKeyPlist(NSString *p, NSString *k, NSDictionary *d) {
    ...
    NSString *tempResult = dRaw(p, encryptedValue);
...
// encryptedSaveNumberForKeyPlist
void secureSaveNumberForKeyPlist(NSString *p, NSNumber *v, NSString *k, NSMutableDictionary *d) {
    // Generate the encrypted key (using the pin and the unencrypted value of the key)
    NSString *encryptedKey = eRaw(p, k);

    // Convert the value to a string
    NSString *valueString = [v stringValue];

    // Encrypt the value associated with this key
    NSString *encryptedValue = eRaw(p, valueString);
